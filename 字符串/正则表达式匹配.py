...
# 够匹配模式串当前位置字符，即 pattern[0] 。如果两种操作中有任何一种使得剩下的字符串能匹配，那么初始时，匹配串和模式串就可以被匹配。

# *---------------------------------------------------------------------------*
# text长度恒定；pattern长度可变（*） => 判断pattern长度
def isMatch(self, text, pattern):
#     pattern为空
    if not pattern:
        return not text

#     text为空/比对第一个字符 => text为空或第一位不匹配则为false
    first_match = bool(text) and pattern[0] in {text[0], '.'}

#     当*在第二位，考虑重复出现的情况
    if len(pattern) >= 2 and pattern[1] == '*':
#         （self.isMatch(text, pattern[2:]): 当*表示0个前一位字符时）
...
# *----------------------------------------------------------------------------------------------------------------------------------*
# 方法 2: 动态规划
# 想法: 因为题目拥有最优子结构 ，一个自然的想法是将中间结果保存起来。我们通过*用 dp(i,j) 表示 text[i:] 和 pattern[j:] 是否能匹配*。我们可以用
# 更短的字符串匹配问题来表示原本的问题。
...
# 复杂度分析:
# -时间复杂度:用 T 和 P 分别表示匹配串和模式串的长度。对于i=0,...,T 和 j=0,...,P 每一个 dp(i, j)只会被计算一次，
